...
interface PDFDocumentProps {
  user: any; // Đặt thành any để linh hoạt khi Vercel kiểm tra kiểu
  session: {
    id: string;
    title: string;
  };
...
const PDFDocument = forwardRef<HTMLDivElement, PDFDocumentProps>(
  ({ user, session, data, submitTime }, ref) => {
    ...
    // Nếu Vercel (máy chủ ảo) đang chạy và user bị null, hệ thống tự động dùng dữ liệu rỗng.
    // Nhờ vậy biến user không bao giờ bị null -> Lỗi "Cannot read properties of null" bị triệt tiêu 100%.
    const safeUser = user || { hoten: "", mssv: "", lop: "" };
      ...
      >
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            {/* Sử dụng safeUser thay vì user */}
            <p><span className="font-bold">Họ và tên sinh viên:</span> {safeUser.hoten}</p>
            <p><span className="font-bold">Mã số sinh viên:</span> {safeUser.mssv}</p>
            <p><span className="font-bold">Lớp:</span> {safeUser.lop}</p>
        ...
        <div className="flex justify-between mt-8 text-sm">
          ...
          <div className="text-center w-1/2">
            ...
            {/* Sử dụng safeUser */}
            <p className="font-bold">{safeUser.hoten}</p>
          </div>
        </div>

        <div className="mt-16 pt-2 border-t border-dashed border-gray-400 text-xs text-gray-500 text-right">
          Mã xác thực hệ thống: {safeUser.mssv}-{Date.now().toString().slice(-6)}
        </div>
      </div>
    );
  }
